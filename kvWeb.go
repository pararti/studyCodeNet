...
func changeElement(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Изменение элементов в хранилище")
	...
	if !CHANGE(key, n){
		fmt.Println("Update operation faild!")
	}else{
		err := save()
		if err != nil {
			fmt.Println(err)
...
func insertElement(w http.ResponseWriter, r *http.Request){
	...
	if !ADD(key, n){
		fmt.Println("Add operation faild!")
	}else{
		err := save()
		if err != nil {
			fmt.Println(err)
...
func main(){
	...
	args := os.Args
	if len(args) == 1 {
		fmt.Println("Используется стандартный порт:", PORT)
	}else{
		PORT = ":" + args[1]
	}
